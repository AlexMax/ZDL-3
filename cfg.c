...
int ReadINI(char* section, char* entry, char* out, int limit, char* ini) {
    ...
    strcpy(sct, "[");
    strcat(sct, section);
    strcat(sct, "]"); // make [sct] = "[*section*]"
    i = (strlen(entry) + 2) * sizeof(char);
    memset((key = malloc(i)), 0, i);
    strcpy(key, entry);
    strcat(key, "="); // make [key] = "*entry*="
    i = (strlen((strlen(sct) > strlen(key)) ? (sct) : (key)) + 1) *
        sizeof(char);
    memset((tmp = malloc(i)), 0, i);
    // Get to the proper section
    do {
        fgets(tmp, strlen(sct) + 1, fptr);
        if (feof(fptr)) {
            i = -2;
            goto exit;
        } // Die if [section] can't be found
    ...
    do {
        ...
        if (feof(fptr) || tmp[0] == '[') {
            i = -3;
            goto exit;
        } // Die if [entry] can't be found in [section]
...
void RegisterFileType(
    char* ext, char* type, char* nicetype, char* exe, char* command, int icon) {
    int i = 0;
    char *tmp = 0, *tmp2 = 0;
...
void Cfg_LoadConfig() {
    ...
    char tmp[8] = {0};
    ReadINI(
        "zdl.general", "alwaysadd", cfg.always, sizeof(cfg.always), cfg.ini);
    ...
    for (i = 0; i < MAX_ITEM; i++) {
        ...
        if (ReadINI("zdl.ports", tmp, port[i]->name, MAX_NAME, cfg.ini) < 0) {
            free(port[i]);
            port[i] = 0;
            break;
        }
        _snprintf(tmp, sizeof(tmp), "p%df", i);
        if (ReadINI("zdl.ports", tmp, port[i]->path, MAX_PATH, cfg.ini) < 0) {
            free(port[i]);
            port[i] = 0;
            break;
        }
    ...
    for (i = 0; i < MAX_ITEM; i++) {
        ...
        if (ReadINI("zdl.iwads", tmp, iwad[i]->name, MAX_NAME, cfg.ini) < 0) {
            free(iwad[i]);
            iwad[i] = 0;
            break;
        }
        _snprintf(tmp, sizeof(tmp), "i%df", i);
        if (ReadINI("zdl.iwads", tmp, iwad[i]->path, MAX_PATH, cfg.ini) < 0) {
            free(iwad[i]);
            iwad[i] = 0;
            break;
        }
...
int Cfg_GetSelStr(char* name, ITEM** item) {
    ...
    for (i = 0;; i++) { // Get to the real index
        if (!item[i] || i >= MAX_ITEM) {
            return -2;
        }
        if (!_stricmp(name, item[i]->name)) {
            break;
        }
    }
    if (!item[i]->avail) {
        return -1;
    } // Make sure the found entry is available
    for (q = 0; q < i; q++) {
        if (item[q]->avail) {
            r++;
        }
    } // Figure out what the list index is
    return r;
}
...
int Cfg_ReadSave(HWND dlg, char* file) {
    ...
    char *tmp = 0, *tmp2 = 0;
    int boxid[2][6] = {
        {LST_WARP, EDT_EXTRA, EDT_HOST, EDT_FRAGS, EDT_DMF, EDT_DMF2},
        {LST_SKILL, LST_GAME, LST_PLAYERS}};
    char* box[2][6] = {
        {"warp", "extra", "host", "fraglimit", "dmflags", "dmflags2"},
        {"skill", "gametype", "players"}};
    ...
    memset(
        (tmp2 = malloc(MAX_PATH * sizeof(char))), 0, MAX_PATH * sizeof(char));
    // Try to find [zdl.save] to see if this is a save file
    if (ReadINI("zdl.save", "keyb", tmp, MAX_PATH, file) == -2) {
        MessageBox(dlg, "This save file is an invalid format.", "Error!",
    ...
    if (ReadINI("zdl.save", "port", tmp, MAX_PATH, file) > 0) {
        if (!port[0] || (i = Cfg_GetSelStr(tmp, port)) < 0) {
            _snprintf(tmp2, MAX_PATH, STR_NEEDPRT, tmp,
                port[Cfg_GetSel(0, port)]->name);
            if (MessageBox(dlg, tmp2, "Port not found!",
    ...
    if (ReadINI("zdl.save", "iwad", tmp, MAX_PATH, file) > 0) {
        if (!iwad[0] || (q = Cfg_GetSelStr(tmp, iwad)) < 0) {
            _snprintf(tmp2, MAX_PATH, STR_NEEDIWD, tmp,
                iwad[Cfg_GetSel(0, iwad)]->name);
            if (MessageBox(dlg, tmp2, "Load Failed!",
                    MB_YESNO | MB_ICONQUESTION) == IDNO) {
                i = 0;
                goto exit;
            }
    ...
    for (i = 0; i < 6; i++) {
        if (ReadINI("zdl.save", box[0][i], tmp, MAX_PATH, file) > 0) {
            SendMessage(
                GetDlgItem(dlg, boxid[0][i]), WM_SETTEXT, 0, (LPARAM)tmp);
        }
    }
    // ComboBoxes (Skill, GameType, Players)
    for (i = 0; i < 3; i++) {
        if (ReadINI("zdl.save", box[1][i], tmp, MAX_PATH, file) > 0) {
            SendMessage(GetDlgItem(dlg, boxid[1][i]), CB_SETCURSEL,
                (WPARAM)atoi(tmp), 0);
        }
    }
    // Dialog Mode
    if (ReadINI("zdl.save", "dlgmode", tmp, MAX_PATH, file) > 0 &&
        !_stricmp(tmp, "open") && cfg.dlgmode) {
    ...
    for (i = 0; i < MAX_PWAD; i++) {
        ...
        if (i >= MAX_PWAD) {
            MessageBox(dlg, STR_MAXPWAD, "Error!", MB_OK | MB_ICONEXCLAMATION);
            break;
        }
        q++;
    }
...
void Cfg_WriteSave(HWND dlg, FILE* fptr) {
    ...
    // Standard stuff
    if (SendMessage(GetDlgItem(dlg, LST_PORT), CB_GETCURSEL, 0, 0) != LB_ERR) {
        fprintf(fptr, "port=%s\n",
            port[Cfg_GetSel(
                     SendMessage(GetDlgItem(dlg, LST_PORT), CB_GETCURSEL, 0, 0),
                     port)]
                ->name);
    }
    if (SendMessage(GetDlgItem(dlg, LST_IWAD), LB_GETCURSEL, 0, 0) != LB_ERR) {
        fprintf(fptr, "iwad=%s\n",
            iwad[Cfg_GetSel(
                     SendMessage(GetDlgItem(dlg, LST_IWAD), LB_GETCURSEL, 0, 0),
                     iwad)]
                ->name);
    }
    if (!cfg.dlgmode) {
    ...
    if (tmp[0] && _stricmp(tmp, "")) {
        fprintf(fptr, "warp=%s\nskill=%d\n", tmp,
            SendMessage(GetDlgItem(dlg, LST_SKILL), CB_GETCURSEL, 0, 0));
    }
    if ((i = SendMessage(GetDlgItem(dlg, LST_GAME), CB_GETCURSEL, 0, 0))) {
        fprintf(fptr, "gametype=%d\n", i);
    }
    if ((i = SendMessage(GetDlgItem(dlg, LST_PLAYERS), CB_GETCURSEL, 0, 0))) {
        fprintf(fptr, "players=%d\n", i);
    }
    for (i = 0; i < 5; i++) { // Grab text from the boxes
        if (SendMessage(GetDlgItem(dlg, boxid[i]), WM_GETTEXTLENGTH, 0, 0)) {
            SendMessage(
                GetDlgItem(dlg, boxid[i]), WM_GETTEXT, MAX_PATH, (LPARAM)tmp);
