...
void Dlg_Launch(HWND dlg, char prompt) {
    ...
    if (!SendMessage(GetDlgItem(dlg, LST_PORT), CB_GETCOUNT, 0, 0) ||
        !SendMessage(GetDlgItem(dlg, LST_IWAD), LB_GETCOUNT, 0, 0)) {
    ...
    portExe = port[Cfg_GetSel(SendMessage(GetDlgItem(dlg, LST_PORT),
                                  CB_GETCURSEL, 0, 0),
                       port)]
                  ->path;
    _snprintf(cmd, q, "\"%s\" -iwad \"%s\"%s%s", portExe,
        iwad[Cfg_GetSel(
                 SendMessage(GetDlgItem(dlg, LST_IWAD), LB_GETCURSEL, 0, 0),
                 iwad)]
            ->path,
        (strlen(cfg.always)) ? (" ") : (""), cfg.always);
    // Warp and Skill
    ...
    if (_stricmp(tmp, "")) {
        ...
        _snprintf(&cmd[strlen(cmd)], q - strlen(cmd), " -skill %d",
            SendMessage(GetDlgItem(dlg, LST_SKILL), CB_GETCURSEL, 0, 0) + 1);
    }
    // PWADs and DEHs
    for (i = 0; pwad[i] && i < MAX_PWAD; i++) { // Count up each type of file
        if (_stricmp(strrchr(pwad[i], '.'), ".deh") &&
            _stricmp(strrchr(pwad[i], '.'), ".bex")) {
            pw++;
        } else {
            pt++;
        }
    ...
    if (pw) { // Append PWADs
        ...
        for (i = 0; pwad[i] && i < MAX_PWAD; i++) {
            if (_stricmp(strrchr(pwad[i], '.'), ".deh") &&
                _stricmp(strrchr(pwad[i], '.'), ".bex")) {
                _snprintf(
                    &cmd[strlen(cmd)], q - strlen(cmd), " \"%s\"", pwad[i]);
            }
        }
    }
    if (pt) { // Append DEHs
    ...
    if (SendMessage(GetDlgItem(dlg, EDT_EXTRA), WM_GETTEXTLENGTH, 0, 0)) {
        ...
        SendMessage(GetDlgItem(dlg, EDT_EXTRA), WM_GETTEXT, q - strlen(cmd),
            (LPARAM)&cmd[strlen(cmd)]);
    }
    // Network stuff
    if (SendMessage(GetDlgItem(dlg, LST_GAME), CB_GETCURSEL, 0, 0)) {
        if ((i = SendMessage(GetDlgItem(dlg, LST_PLAYERS), CB_GETCURSEL, 0,
                 0))) { // Hosting
            ...
            if (SendMessage(GetDlgItem(dlg, LST_GAME), CB_GETCURSEL, 0, 0) ==
                ...
                // Fragimit
                if (SendMessage(
                        GetDlgItem(dlg, EDT_FRAGS), WM_GETTEXTLENGTH, 0, 0)) {
                    strncat(cmd, " +fraglimit ", q - strlen(cmd));
                    SendMessage(GetDlgItem(dlg, EDT_FRAGS), WM_GETTEXT,
                        q - strlen(cmd), (LPARAM)&cmd[strlen(cmd)]);
                }
            }
            // DMF
            if (SendMessage(GetDlgItem(dlg, EDT_DMF), WM_GETTEXTLENGTH, 0, 0)) {
                strncat(cmd, " +dmflags ", q - strlen(cmd));
                SendMessage(GetDlgItem(dlg, EDT_DMF), WM_GETTEXT,
                    q - strlen(cmd), (LPARAM)&cmd[strlen(cmd)]);
            }
            // DMF2
            if (SendMessage(
                    GetDlgItem(dlg, EDT_DMF2), WM_GETTEXTLENGTH, 0, 0)) {
                strncat(cmd, " +dmflags2 ", q - strlen(cmd));
                SendMessage(GetDlgItem(dlg, EDT_DMF2), WM_GETTEXT,
                    q - strlen(cmd), (LPARAM)&cmd[strlen(cmd)]);
            }
        } else { // Joining
            if (!SendMessage(
                    GetDlgItem(dlg, EDT_HOST), WM_GETTEXTLENGTH, 0, 0)) {
                MessageBox(dlg, "You need to type a host to join", "Error!",
                    MB_OK | MB_ICONHAND);
                free(cmd);
                return;
            }
            strcat(cmd, " -join ");
            SendMessage(GetDlgItem(dlg, EDT_HOST), WM_GETTEXT, q - strlen(cmd),
                (LPARAM)&cmd[strlen(cmd)]);
        }
    }
    if (prompt && MessageBox(dlg, cmd, "Run this command-line?",
...
int Dlg_AddPWAD(HWND dlg, char* file) {
    ...
    if (i >= MAX_PWAD || !strrchr(file, '.') ||
        !_stricmp(strrchr(file, '.'), ".zdl") ||
        !_stricmp(strrchr(file, '.'), ".ini")) {
        return 0;
    }
    memset((pwad[i] = malloc((MAX_PATH + 1) * sizeof(char))), 0,
        MAX_PATH * sizeof(char));
    strncpy(pwad[i], file, MAX_PATH);
    SendMessage(GetDlgItem(dlg, LST_PWAD), LB_ADDSTRING, 0,
        (strrchr(file, '\\')) ? ((LPARAM)strrchr(file, '\\') + 1)
                              : ((LPARAM)file));
    return 1;
}
/////////////////////////////////////////////////
...
void Dlg_ClearAll(HWND dlg) {
    int i = 0;
    int box[2][5] = {{LST_PORT, LST_GAME, LST_PLAYERS},
        {EDT_EXTRA, EDT_HOST, EDT_FRAGS, EDT_DMF, EDT_DMF2}};
    Dlg_ClearPWAD(dlg);
    ...
    for (i = 0; i < 3; i++) {
        SendMessage(GetDlgItem(dlg, box[0][i]), CB_SETCURSEL, 0, 0);
    }
    for (i = 0; i < 5; i++) {
        SendMessage(GetDlgItem(dlg, box[1][i]), WM_SETTEXT, 0, (LPARAM) "");
    }
}
...
void Dlg_Quit(HWND dlg, char save) {
    ...
    // Save the config
    if (save) {
        if (!cfg.always[0] && !cfg.launch && !port[0] &&
            !iwad[0]) { // Do we even need to save at all?
            Dlg_ClearPWAD(dlg);
            EndDialog(dlg, 0);
            return;
        }
        if (!(fptr = fopen(cfg.ini, "w"))) {
            MessageBox(HWND_DESKTOP,
                "The INI file could not be opened for writing!\nYour settings "
                "have not been saved!",
    ...
    if ((fptr = fopen(file, "rb"))) {
        ...
        for (i = 0; i < header.lumps; i++) {
            ...
            if (!_stricmp(lump[1].name, "THINGS")) {
                ...
                SendMessage(
                    GetDlgItem(dlg, LST_WARP), CB_ADDSTRING, 0, (LPARAM)temp);
            } else {
                fseek(
                    fptr, sizeof(LUMPHEAD) - (sizeof(LUMPHEAD) * 2), SEEK_CUR);
            }
        }
